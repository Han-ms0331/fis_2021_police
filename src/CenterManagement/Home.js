...
function Home(props) {
	...
	const getSearchCenterList_c = async (search) => {  //search : 날짜
		console.log(props.uid);

		const result = await axios.get(
			// `http://192.168.0.117:3000/home/date/${props.uid}/${search}`
		);
		setResult_1ary(result);
		resettingRef.current = true;
		console.log(result);
	...
	const getSearchCenterList_d = async (search) => {  //search : 기록자 이름
		console.log(props.uid);

		const result = await axios.get(
			// `http://192.168.0.117:3000/home/recorder/${props.uid}/${search}`
		);
		setResult_1ary(result);
		resettingRef.current = true;
		console.log(result);
						...
						<div class='main_search_box'>
							...
							<div class='main_search_window'>
								...
								<input
									...
									class='main_serch_input'
									onChange={onChange}
									onKeyPress={onKeyPress_c}
								/>
								<button class='main_search_btn' onClick={onClick_c}>
									검색
								</button>
							</div>
							<div class='main_search_window'>
